...
-- EXPECTED OUTPUT: 146
SELECT COUNT(id) AS 'Number of Rows in Table' FROM final_airbnb;
...
SELECT COUNT(DISTINCT neighbourhood) AS "Neighborhoods Listed" FROM final_airbnb;
...
-- Perhaps not since it is tied with Harlem

SELECT neighbourhood,count(*) FROM final_airbnb GROUP BY neighbourhood ORDER BY count(*) DESC LIMIT 1;
...
SELECT host_name AS 'Host Name', count(*) AS 'Number of units'  FROM final_airbnb GROUP BY host_name ORDER BY count(*) DESC;
